!function () {
	...
	Donut3D.transition = function (id, data, rx, ry, h, ir) {
		...
		d3.select("#" + id).selectAll(".innerSlice").data(_data)
			.transition().duration(duration).ease(easetype).attrTween("d", arcTweenInner);

		d3.select("#" + id).selectAll(".topSlice").data(_data)
			.transition().duration(duration).ease(easetype).attrTween("d", arcTweenTop);
	...
	Donut3D.draw = function (id, data, x /*center x*/, y/*center y*/,
		...
		slices.selectAll(".innerSlice").data(_data).enter().append("path").attr("class", "innerSlice")
			.style("fill", function (d) { return d3.hsl(d.data.color).darker(0.7).toString(); })
		...
		var pielegend = d3.select("#fullpie")
			.append("g")
		...
		pielegend
			...
			.attr("width", 19)
			.attr("height", 19)
			.attr("fill", function(d) {return d.data.color.toString()});
